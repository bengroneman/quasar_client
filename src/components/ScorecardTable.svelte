						  ...
						  {#each rows as row, index}
						    {#if !(department_names.indexOf(row.dept_name) > -1)}
							  <tr class="border-gray-100 bg-gray-50 table-row">
							  	<th colspan="15" scope="colgroup" class="table-header font-display text-orange-soda uppercase tracking-wide">
							...
							<tr class="table-row">
							  ...
							  {#each row.metrics as metric}
								...
								{:else}
								  <td class={colorCodeCell(measure_rows[index].goal, metric, measure_rows[index].good)}>
								    <span>{metric}</span>
								  </td>
								{/if}
							  {/each}
